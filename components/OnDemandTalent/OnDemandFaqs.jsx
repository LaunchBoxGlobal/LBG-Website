...
import { ON_DEMAND_FAQS } from "@/constants/on-demand-talent-content/on-demand-faqs";
...
const OnDemandFaqs = () => {
  ...
  return (
    <section
      className="w-full py-10 lg:pt-20 lg:pb-32 padding-x flex flex-col items-center overflow-hidden"
      id="faqs"
    >
      ...
      <section className="w-full mt-10 lg:mt-24 flex flex-col items-start gap-4 lg:w-[90%]">
        {ON_DEMAND_FAQS?.map((faq, index) => (
          <div
            className={`w-full mx-auto rounded-3xl px-5 py-5 lg:px-10 lg:py-14 bg-[#F9F9F9] text-black`}
            key={index}
          >
            <button
            ...
            >
              <h5 className="font-medium text-[17px] md:text-2xl lg:text-[35px] 2xl:text-[40px] w-[90%]">
                <span className="w-full leading-[1] lg:leading-[1.3]">
                  {faq?.question}
                </span>
              </h5>
