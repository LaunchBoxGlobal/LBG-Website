import { MOBILE_APP_DEVELOPMENT_SECURITY_SOLUTIONS } from "@/constants/mobile-app-development/mobile-app-development-security-solutions";
    ...
    <section className="w-full padding-x py-10 lg:py-20 relative overflow-hidden">
      <section className="w-full flex flex-col items-center justify-center gap-6 text-center">
        <h2 className="section-heading w-full">
          ...
          <span className="red-text">Mobile Applications</span>
        </h2>
        <p className="section-paragraph lg:w-[85%]">
          Security is at the heart of every mobile app we build. As a trusted
          Mobile app development company, we create resilient mobile apps that
      ...
      <section className="w-full mt-10 lg:mt-14 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {MOBILE_APP_DEVELOPMENT_SECURITY_SOLUTIONS?.map((model, index) => {
          return (
            <div key={index} className="w-full">
              <div className="w-full lg:min-h-[410px] midlg:h-[450px] bg-[#F3F3F3] rounded-[17px] p-7 midlg:p-10 flex flex-col items-center gap-4">
                <img
                  src={model?.icon}
                  width={114}
                  height={114}
