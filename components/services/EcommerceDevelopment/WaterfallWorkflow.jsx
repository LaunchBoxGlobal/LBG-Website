...
const WaterfallWorkflow = ({ workflow }) => {
  return (
    <section className="w-full mt-10 lg:mt-20 lg:mb-20 relative">
      <Image
      ...
      <Image
        ...
        height={221}
        loading="lazy"
        className="object-contain absolute lg:bottom-[-3%] midlg:bottom-[-2%] xl:bottom-[-2%] lg:right-[1%] xl:right-[4%] w-[219px] h-[221px] hidden lg:block"
      />
      {workflow?.map((plan, index) => {
        ...
        return (
          <div
            ...
            key={index}
          >
            <p className="font-medium text-base md:text-[17px]">
              {plan?.tag_line}
            </p>
            <h3 className="font-medium text-[22px] md:text-[30px] red-text">
              {plan?.title}
            </h3>
            <p className="text-[16px] md:text-lg">{plan?.description}</p>
            <div
