  ...
  return (
    ...
    >
      ...
      <section className="w-full mt-10 lg:mt-24 flex flex-col items-start gap-4 lg:w-[90%]">
        {BRANDING_AND_DESIGN_FAQS?.map((faq, index) => (
          <div
        ...
        <div className="w-full rounded-3xl px-5 py-5 lg:px-10 lg:py-14 bg-[#F9F9F9] text-black">
          <button
            onClick={() => toggleFaq(BRANDING_AND_DESIGN_FAQS.length)}
            className="w-full text-start flex items-start justify-between outline-none"
          >
            <h3 className="font-medium text-[17px] md:text-2xl lg:text-[35px] 2xl:text-[40px] w-[90%]">
              How does branding influence the success of eCommerce development
              projects?
            </h3>
            ...
            <Image
              ...
              alt="arrow icon"
              className={`${
                openFaq === BRANDING_AND_DESIGN_FAQS.length
                  ? "scale-y-[-1]"
                  : "scale-y-[1]"
          ...
          <motion.div
            initial={false}
            animate={{
              height: openFaq === BRANDING_AND_DESIGN_FAQS.length ? "auto" : 0,
              opacity: openFaq === BRANDING_AND_DESIGN_FAQS.length ? 1 : 0,
              marginTop:
                openFaq === BRANDING_AND_DESIGN_FAQS.length ? "18px" : 0,
            }}
            transition={{ duration: 0.5, ease: "easeInOut" }}
            className="overflow-hidden"
          >
            <p className="text-base lg:text-[20px] font-normal lg:w-[90%]">
              Branding plays a critical role in the success of any eCommerce
              platform—it defines how customers perceive your store, builds
              trust, and influences buying decisions. At Launchbox Global, we
              ensure that every element—from your logo and color palette to UI
              design and messaging—is aligned to create a seamless and memorable
              shopping experience. When paired with our{" "}
              <Link
              ...
              , your brand identity becomes a revenue-driving asset across every
              digital touchpoint.
            </p>
          </motion.div>
        </div>
